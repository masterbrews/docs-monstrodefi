...
export function generateStaticParams(): Params[] {
  return getAllUrls().map((u) => ({ slug: u === '/' ? [] : u.slice(1).split('/') }))
}
...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  const url = page.url
  return {
    title: url === '/' ? { absolute: `${page.title} | ${config.name}` } : page.title,
    description: page.description ?? undefined,
    alternates: { canonical: url },
    robots: page.hidden ? { index: false, follow: true } : undefined,
    openGraph: { title: page.title, description: page.description ?? undefined, url },
  }
}
...
export default async function Page({ params }: { params: Promise<Params> }) {
  ...
  return (
    <div className="page-wrap">
      {page.cover ? (
        <div className="page-cover" style={{ '--cover-y': `${-page.coverY}px` } as React.CSSProperties}>
          <img src={page.cover.src} width={page.cover.width} height={page.cover.height} alt="" fetchPriority="high" />
        </div>
      ) : null}
