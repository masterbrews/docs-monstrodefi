...
import { faMagnifyingGlass, faXmark, faChevronRight, faFileLines, faArrowTurnDown, faArrowUp, faArrowDown } from '@fortawesome/free-solid-svg-icons'
    ...
    indexPromise = fetch('/search-index.json')
      ...
      .then((entries: Entry[]) => {
        const ms = new MiniSearch<Entry>({
          ...
          storeFields: ['url', 'title', 'crumbs', 'description', 'heading', 'text'],
...
export function Search() {
  ...
  const terms = useMemo(() => query.trim().split(/\s+/).filter((t) => t.length > 1), [query])
  ...
  return (
    <>
      ...
      {searchOpen ? (
        <>
          <div className="search-overlay" onClick={() => setSearchOpen(false)} aria-hidden="true" />
          <div className="search-box" role="search">
            ...
            {query ? (
              <>
                {ready ? <span className="search-count">{hits.length} result{hits.length === 1 ? '' : 's'}</span> : null}
                <button type="button" className="search-clear" aria-label="Clear" onClick={() => setQuery('')}>
                  <FaIcon icon={faXmark} />
                </button>
              </>
            ) : (
              <button type="button" className="search-clear" aria-label="Close" onClick={() => setSearchOpen(false)}>
                <FaIcon icon={faXmark} />
              </button>
            )}
          </div>
            ...
            <div className="search-results" role="listbox" aria-label="Search results">
              {hits.length ? (
                <ul className="search-list" ref={listRef}>
                  {hits.map((hit, i) => (
                    <li key={hit.id} className="search-item" role="option" aria-selected={i === selected} data-index={i}>
                      <a
                        href={hit.url}
                      ...
                      >
                        ...
                        <span className="search-item-main">
                            ...
                            <span className="search-item-crumbs">
                              {crumbsOf(hit).map((c, j) => (
                                <span key={j} style={{ display: 'contents' }}>
                                  {j > 0 ? <FaIcon icon={faChevronRight} /> : null}
                                  <span>{c}</span>
                                </span>
                              ))}
                            </span>
                          ...
                          <span className="search-item-title">
                            ...
                            {hit.heading && hit.heading !== hit.title ? <> · {highlight(hit.heading, terms)}</> : null}
                          </span>
                          <span className="search-item-snippet">{highlight(snippet(hit, terms), terms)}</span>
                        </span>
                        <span className="search-item-go" aria-hidden="true">
                          <FaIcon icon={i === selected ? faArrowTurnDown : faChevronRight} />
                        </span>
                      </a>
                    </li>
                  ))}
                ...
                <div className="search-empty">{ready ? `No results for “${query.trim()}”` : 'Loading…'}</div>
              )}
