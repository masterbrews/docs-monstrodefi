...
import { parseSummary, flattenPages, readPageFile, splitTitle, type NavItem, type FlatPage } from './summary.mjs'
import { renderMarkdown, assetKey, type TocEntry, type AssetInfo } from './markdown.mjs'
...
export function getSiteIndex(): SiteIndex {
  ...
  cached = {
    ...
    meta: readJson(path.join(siteDir, 'generated', 'meta.json'), { pages: {}, builtAt: Math.floor(Date.now() / 1000) }),
  }
  return cached
}
...
export function getNav(): NavNode[] {
  ...
  const walk = (items: NavItem[]): NavNode[] =>
    items
      ...
      .map((i) => ({ title: i.title, url: i.url, href: i.href, icon: iconOf(idx, i), group: i.group, children: walk(i.children || []) }))
...
export async function getPage(url: string): Promise<PageData | null> {
  ...
  const nextItem = pos >= 0 && pos < visible.length - 1 ? visible[pos + 1] : null
  const titleOf = (p: FlatPage) => splitTitle(readPageFile(contentDir, p.file!).content).title || p.title
  ...
  return {
    ...
    html,
    toc,
    breadcrumbs: item.parents.map((p) => ({ title: p.title, url: p.url!, icon: iconOf(idx, p) })),
